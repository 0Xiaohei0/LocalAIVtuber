from plugins.rvc.lib.infer_pack.commons import get_padding, init_weights
import plugins.rvc.lib.infer_pack.attentions as attentions
import plugins.rvc.lib.infer_pack.commons as commons
import plugins.rvc.lib.infer_pack.modules as modules
...
class GeneratorNSF(torch.nn.Module):
    def __init__(
        ...
        self.f0_upsamp = torch.nn.Upsample(
            scale_factor=math.prod(upsample_rates))
...
class SynthesizerTrnMs256NSFsid(nn.Module):
    ...
    def infer(
        ...
        z_p = (m_p + torch.exp(logs_p) *
               torch.randn_like(m_p) * 0.66666) * x_mask
        if rate is not None:
