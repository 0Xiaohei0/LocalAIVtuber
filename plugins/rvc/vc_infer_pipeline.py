...
class VC(object):
    ...
    def get_f0(
        ...
        if f0_method == "pm":
            ...
            f0 = cache_harvest_f0(
                input_audio_path, self.sr, f0_max, f0_min, 10)
            if filter_radius > 2:
            ...
            if hasattr(self, "model_rmvpe") == False or self.model_rmvpe == None:
                from plugins.rvc.rmvpe import RMVPE

                print("loading rmvpe model")
                self.model_rmvpe = RMVPE(
                    f"{os.path.join(os.path.dirname(__file__), 'rmvpe.pt')}", is_half=self.is_half, device=self.device
                )
