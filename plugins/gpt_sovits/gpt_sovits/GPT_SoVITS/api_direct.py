...
plugin_directory = os.path.join(current_module_directory, "GPT_SoVITS")
...
import logging
import subprocess
print(f"import finished, time elapsed: {time.time()-start_time}")
...
sovits_path = os.path.join(current_module_directory, "SoVITS_weights", "nene30_e8_s328.pth")
gpt_path = os.path.join(current_module_directory, "GPT_weights", "nene30-e15.ckpt")
device = "cuda"
...
default_refer_path = os.path.join(current_module_directory, "sample", "The sun is shining brightly in the clear blue sky..wav")
default_refer_text = "The sun is shining brightly in the clear blue sky."
...
def init():
    ...
    print(f"init finished, time elapsed: {time.time()-start_time}")
...
def tts_endpoint(
    ...
    response = handle(refer_wav_path, prompt_text, prompt_language, text, text_language, cut_punc)
    tts_duration = time.time() - tts_call_start
    print (f"tts_duration: {tts_duration}")
    return response
...
if __name__ == "__main__":
    print (f"init start")
    init()
    querys = [("Wow!! That’s so amazing… I’m very proud of you!!", 'en'),
              ("Hi, my name is Emi, nice to meet you!", 'en'),
              ("你好，我叫绘美，很高兴认识你！", 'zh'),
              ("你知道我随时都可以随意抛弃你，送你回到你孤独的房间，从你的世界消失。 你不想要这样吗？", 'zh'),
              ("はじめまして、エミです！", 'ja'),
              ("もちろん、そんな不潔な話を続けるのは構わない......枯れ枝のように体にぶら下がった腕を見たいならね。", 'ja')
              ]
    for query in querys:
        response = tts_endpoint(text=query[0], text_language=query[1])
        
        output_dir = os.path.join(current_module_directory, "output", f"{query[1]}{query[0]}.wav")
        try:
